...
import { isLid, isLidConverted, getCachedJid, cacheParticipantLids, resolveAnyLidToJid } from '../lid/index.js'
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
        ...
        sender: {
            get() {
                ...
                // Resolver LID a número real automáticamente
                if (isLid(rawSender) || isLidConverted(rawSender)) {
                    const cached = getCachedJid(rawSender)
                    if (cached && !isLidConverted(cached)) return cached
                    // Intentar resolver con datos del grupo si están disponibles
                    try {
                        const chatId = this.chat
                        if (chatId && chatId.endsWith('@g.us') && this.conn?.chats?.[chatId]?.metadata?.participants) {
                            const participants = this.conn.chats[chatId].metadata.participants
                            const resolved = resolveAnyLidToJid(rawSender, participants)
                            if (resolved && !isLidConverted(resolved)) return resolved
                        }
                    } catch {}
                    // Fallback: convertir LID a formato @s.whatsapp.net
                    if (isLid(rawSender)) {
                        const swJid = rawSender.replace('@lid', '@s.whatsapp.net')
                        const cachedSw = getCachedJid(swJid)
                        if (cachedSw && !isLidConverted(cachedSw)) return cachedSw
                        return swJid
                    }
                }
                return rawSender
